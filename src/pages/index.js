	...
	return (
		<>
			...
			<form className="relative mx-auto flex max-w-lg" onSubmit={handleSubmit}>
				...
				<input
					className="w-full rounded-md border border-gray-300 pr-28 pl-10 placeholder:text-gray-400 focus:border-gray-400 focus:ring-0"
					type="text"
					name="url"
					id="url"
				...
				<button
					className="absolute inset-y-0 right-0 flex items-center justify-center rounded-md bg-black p-4 text-white transition-all hover:opacity-80"
					type="submit"
				>
					Check
				</button>
				...
				<div className="border-1 mx-auto my-6 max-w-lg rounded-md border border-gray-200 bg-white p-4 shadow-xl">
					{ogMetadata.ogImage ? (
						...
						<div className="mx-auto flex aspect-video select-none items-center justify-center rounded border border-gray-100 bg-gray-100">
							<code className="mx-1 rounded bg-gray-200 p-1">og:image</code> not
							found
						</div>
					)}
